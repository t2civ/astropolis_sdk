...
Tables are imported using I, Voyager's Table Importer plugin. For formatting info, see [readme](https://github.com/ivoyager/ivoyager_table_importer/blob/master/README.md).

As of 2024, most table data is generated by AI queries using Clause-3-Opus (mainly) or -Haiku. Useful chat responses are in tables/README_AI_CHAT.md. Links are sometimes provided for data verification. 
...
* We do have conservation of mass, roughly speaking, but it is achieved statistically. For example, we don't remove 0.000001% of iron ore from a strata for some interval of extraction. Instead, we would remove 0.1% from that strata with a probablility of 0.00001.
...
Values are "rate units" corresponding to quantities in the operations.tsv row. In general, rate = 1.0 means:
...
One "rate unit" corresponds to quantities in the operations.tsv row. In general, rate = 1.0 means:

    ENERGY        - 1 MW electrical output (ie, MWe)
    EXTRACTION    - 1 t/hr extracted ore x (deposits/100 if deposits, or mass fraction)
    BIOME         - 1 km^2 equivilant Earth area
    REFINING      - 1 t/hr total mass conversion (=input or output, always same)
    MANUFACTURING - 1 t/hr total production (= mass conversion as above)
    SERVICES      - 1 unit/hr of whatever intangible resource(s) is(are) produced

#### SOLAR_POWER, WIND_POWER, TIDAL_POWER, HYDROPOWER, GEOTHERMAL_POWER

Rate unit is 1 GW by definition. Utilization is subject to the environment.  
