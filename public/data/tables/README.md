...
As of 2024, most table data is generated by AI querries (currently using Clause-3-Haiku or -Opus) unless indicated otherwise. Links are sometimes provided for data verification. 
...
xxxx = _capacities and _utilizations.
...
Below were generated in a separate Opus thread looking at different renewables. Solar values are a bit different but close to above. Using below for wind, hydro, tidal & geothermal. 2010 capacity (game start) and 2020 efficiency/utilization (fixed), except we use 0.75 for all geothermal.
...
Per Opus:
* ISS/USOS segment generates 84-120 kW and is capable of 8 x 32.8 kW = 262 kW. (This is roughly consistent with 50% capacity factor. We use 200 kW capacity.)
* ISS/Zvezda segment (Roscosmos) generates 28 kW. (We take this as actual, so use 2x for capacity.)
* "Here's what we know..." Expected to generate 20 kW. (We take this as actual, so use 2x for capacity.) 
